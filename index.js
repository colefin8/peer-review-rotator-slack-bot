...
const { BOT_TOKEN, SECRET, CHANNEL, EXCLUDED_USERS } = process.env;
...
// The EXCLUDED_USERS env variable contains user IDs separated by spaces
const excludedUsers = EXCLUDED_USERS?.split(' ') ?? [];
const excludeIds = {};

// Initialize the excludeIds for each channel
if (channels && channels.length > 0) {
    channels.forEach(channel => {
        excludeIds[channel] = excludedUsers;
    });
}
